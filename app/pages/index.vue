...
<template>
    <div class="p-8">
        ...
        <div v-if="user">
            <p class="mb-2"
                >Signed in as:
                <span class="font-mono">{{ user.email }}</span></p
            >
            <p v-if="user.user_metadata && user.user_metadata.display_name"
                >Display Name:
            ...
            >
            <p
                >User ID: <span class="font-mono">{{ user.id }}</span></p
            >
            <div class="mt-4">
                <p v-if="loadingStreak">Loading streak...</p>
                <p v-else-if="streakError" class="text-red-500"
                    >Error: {{ streakError }}</p
                >
                <p v-else
                    >🔥 Current Streak:
                    <span class="font-mono"
                        >{{ streak }}
                        <span v-if="streak === 0">(fuck you 🖕)</span
                        ><span v-else>(yipee! 🙌 app user found :3)</span></span
                    ></p
                >
            </div>
        </div>
