// Composable to manage friendships.
// Depends on a table public.friendships(id bigserial PK, user_id uuid FK profiles.id, friend_id uuid FK profiles.id, created_at timestamptz default now()).
// RLS suggestions (SQL to create, add manually in Supabase SQL editor):
...
//  create policy "own rows" on public.friendships for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
//  -- Optional: allow reciprocal visibility so a user can see rows where they are friend_id
//  create policy "visible to friend" on public.friendships for select using (auth.uid() = friend_id);
//  -- You may also want a helper view for symmetric friendships.
...
// create policy "own request rows" on public.friend_requests for all using (auth.uid() in (requester_id, recipient_id)) with check (auth.uid() = requester_id);
// (Optional) create a view friendships as accepted symmetric relationships.
...
export function useFriends() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data: accepted, error: accErr } = await supabase
        ...
        .select('id, status, created_at, requester_id, recipient_id, requester:profiles!friend_requests_requester_id_fkey(id, display_name, avatar_url, total_points, state), recipient:profiles!friend_requests_recipient_id_fkey(id, display_name, avatar_url, total_points, state)')
        .or(`requester_id.eq.${user.value.id},recipient_id.eq.${user.value.id}`)
        .eq('status','accepted');
      ...
      const { data: inc, error: incErr } = await supabase
        .from('friend_requests')
        .select('id, created_at, requester:profiles!friend_requests_requester_id_fkey(id, display_name, avatar_url, total_points, state)')
        .eq('recipient_id', me)
        .eq('status','pending');
      ...
      const { data: out, error: outErr } = await supabase
        .from('friend_requests')
        .select('id, created_at, recipient:profiles!friend_requests_recipient_id_fkey(id, display_name, avatar_url, total_points, state)')
        .eq('requester_id', me)
        .eq('status','pending');
      if (outErr) throw outErr;
      outgoing.value = (out||[]).map((r:any)=>({
        id: r.recipient?.id,
      ...
      if (e?.code === '42P01') { // undefined_table
        try {
          const { data: fr, error: frErr } = await supabase
            .from('friendships')
            .select('id, friend_id, created_at, profiles:profiles!friendships_friend_id_fkey(id, display_name, avatar_url, total_points, state)')
          ...
          incoming.value = [];
          outgoing.value = [];
          error.value = 'Friend requests table missing; using legacy friendships data. Apply migration to enable requests.';
        } catch (legacyErr: any) {
          error.value = legacyErr.message || 'Failed to load friends';
        } finally {
          loading.value = false;
        }
  ...
  async function addFriend(friendId: string) {
    ...
    const { error: insErr } = await supabase.from('friend_requests').insert({ requester_id: user.value.id, recipient_id: friendId });
    if (insErr) {
  ...
  async function removeFriend(friendId: string) {
    ...
    const { error: delErr } = await supabase
      .from('friend_requests')
      .delete()
      .or(`and(requester_id.eq.${user.value.id},recipient_id.eq.${friendId}),and(requester_id.eq.${friendId},recipient_id.eq.${user.value.id}))`)
      .eq('status','accepted');
    if (delErr) {
  ...
  async function decline(friendId: string) {
    ...
    const { error: delErr } = await supabase
      ...
      .eq('requester_id', friendId)
      .eq('status','pending');
    if (delErr) { error.value = delErr.message; } else { await load(); }
  }
    ...
    try {
      ...
      searchResults.value = (data || [])
        ...
        .map((p: any) => ({
          ...
          created_at: '',
          you: false,
          alreadyFriend: existingIds.has(p.id) || incoming.value.some(r=>r.id===p.id) || outgoing.value.some(r=>r.id===p.id),
        }));
    } catch (e: any) {
  ...
  return { friends, incoming, outgoing, loading, error, reload: load, addFriend, removeFriend, accept, decline, search, searchResults, searching, searchError, pendingCount };
}
